...
function getCannedResponse(query: string): string {
  if (
    query.includes("soap") &&
    (query.includes("note") || query.includes("generate") || query.includes("sample"))
  ) {
  ...
  if (
    ...
    query.includes("on-prem") ||
    query.includes("run inside") ||
  ...
  if (
    query.includes("optomate") ||
    query.includes("ehr") ||
    query.includes("emr") ||
    query.includes("integration") ||
    query.includes("practice management")
  ) {
...
export async function POST(req: Request) {
  ...
  if (!apiKey) {
    ...
    return NextResponse.json({ reply: getCannedResponse(lastMsg) });
  }
